...
.clear1{
	...
	#comment{width:100%;padding-top:10px;}		
		#comment ul{width:100%;}
			#comment ul li{width:90%; margin: auto;}
			#commentInfo{height:40px;margin-left: 20px;font-size: 16px;font-weight: 600;padding-top: 15px;}
			...
			#commentContainer{background-color:#fff;border-top:1px solid #eee;}
				#commentbox{width:100%;margin:0 auto;padding-bottom:10px;min-height: 80px;margin-top: 10px;}
					#cbPlus{width:100%;height:40px;line-height:30px;overflow:hidden;border-bottom:1px solid #eee; margin-top: 6px;}						
						#cbPlus img{height: 30px; vertical-align: middle; } 
						 #cbUser{display: inline-block;height: 30px;}
						 #cbUser img{height:14px; width:14px;cursor:pointer;}
						#cmtModify{display:inline-block;height:15px;line-height:12px;cursor:pointer; float:right; margin-right: 10px; }
					#cmtContent{width:100%;min-height:40px;padding-bottom:10px;padding-top:10px;border-bottom:1px solid #eee;text-align:left;}
			#commentWrite{padding-top:20px;}
				#cwArea{width:100%;margin:0 auto;overflow:hidden;}
					#cwArea p{float:left;}
						#cwText{width:90%;text-align:left;}
							#cwText textarea{width:99%;height:75px;padding-top:5px;padding-left:1%;overflow:auto;resize: none;}
						#cwTSend{width:10%;height:82px;font-size:12px;}
...
#nextPrev a{display: inline-block; float:left; padding: 10px 15px;margin: 10px;font-weight:600;background-color:#f7f7f7; vertical-align: middle; border: 1px solid #ebebeb; border-radius: 5px;}
#writeSub {display: inline-block; float:right; padding: 10px 15px;margin: 10px;cursor:pointer;font-weight:600;background-color:#f7f7f7; vertical-align: middle; border: 1px solid #ebebeb; border-radius: 5px;}
...
<body>
	<div id="wrapper">
		...
		<main id="container">
			...
			<ul id="mainTitle" class="title">
				...
				<li>${detail.sm_name }<small>(${detail.sm_id })</small></li>
					...
					<c:if test="${detail.sb_modifydate ne null }">
						<small style="float: right; margin-right: 15px; margin-top: 3px;">${detail.sb_modifydate } (수정)</small>	
						<br>			
					</c:if>
			...
			<div id="bbottom"><c:if test="${detail.sb_file ne null }">
			...
			<div class="likeDislike">
				<div class="button">
					<a href='#'
						...
						src="./img/smile.png" alt="좋아요"><br>좋아요</a>
						<br>${detail.sb_like }
				</div>
				&nbsp;&nbsp;
				<div class="button">
					<a href='#'
						...
						src="./img/frown.png" alt="싫어요"><br>싫어요</a>
						<br>${detail.sb_dislike }
				</div>
			</div>
			<div id=></div>
			<div id="comment">
				...
				<ul>
						...
						<li id="commentWrite">
							<div id="cwID" style="margin-bottom: 10px;"><img src="./img/info.png" alt="info" style="cursor:pointer; vertical-align: middle; height: 30px;">&nbsp;${sessionScope.sm_name}(${sessionScope.sm_id})</div>
							<div id="cwArea">
					...
					<c:choose> 
						<c:when test="${detail.commentcount gt 0 }">
							<li id="commentInfo">${detail.commentcount }개의 댓글</li>
								...
								<c:choose>
									<c:when test="${true }">
										<c:forEach items="${comment }" var="cmt">
											<div id="commentbox">
												<div id="cbPlus">
													<img src="./img/info.png" alt="info" style="cursor:pointer;">
													<div id = "cbUser">${cmt.sm_name}(${cmt.sm_id })</div>
													<div id = "cbUser">${cmt.sc_date }<c:if test="${cmt.sc_modifydate ne null }"><small>&emsp;${cmt.sc_modifydate } (수정)</small></c:if> </div>
													<div id = "cbUser"><img src="./img/like.png" alt="좋아요">&nbsp;${cmt.sc_like }</div>
												</div>
												...
												<div class="modifyInputP">
													...
													<c:if test="${sessionScope.sm_id eq cmt.sm_id}">
														<button id = cmtModify class="clear1">수정하기</button>
 														<div class="fno" style="display: none;">${cmt.sb_no }</div>
														<div class="fcno" style="display: none;">${cmt.sc_no }</div>
														<form action="./commentDelete" method="post">
														<input type="hidden" name="sb_no" value="${cmt.sb_no }"><input type="hidden" name="cno" value="${cmt.sc_no }">
														<input type="hidden" name="sb_cate" value="${detail.sb_cate }">
													...
													<div class="modifyWrite">
														...
														<textarea name="sc_comment" style="width:100%;height:100px;overflow:auto;resize: none;">${cmt.sc_comment }</textarea>
			...
			<div id="nextPrev">
                     <c:if test="${detail.sb_nno gt 1}">
                        <a href="./nextPrev?option=prev&sb_nno=${detail.sb_nno }&sb_cate=${detail.sb_cate}" title="이전글">이전글</a>               
                     </c:if>
                     <c:if test="${detail.sb_nno lt detail.totalList}">
                        <a href="./nextPrev?option=next&sb_nno=${detail.sb_nno }&sb_cate=${detail.sb_cate}" title="다음글" >다음글</a>                  
                     </c:if>
                 </div>
				<c:if test="${sessionScope.sm_id ne null }">
					<div id="writeSub">
